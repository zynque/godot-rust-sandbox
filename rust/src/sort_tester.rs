...
// Each test case is (label, input slice). The expected output is the
...
impl INode for SortTester {
    ...
    fn ready(&mut self) {
        ...
        godot_print!("[SortTester] Results: {}/{} passed.", passed, passed + failed);
...
fn compile_pipeline(rd: &mut Gd<RenderingDevice>) -> Option<(Rid, Rid)> {
    let shader_file: Gd<RdShaderFile> = load("res://addons/beatsmr/shaders/sort_test.glsl");
...
fn run_test_case(
    ...
    let buffer_rid = rd.storage_buffer_create_ex(byte_count).data(&packed).done();
